const CONFIG = {
  api_key: "fc38338f58588b810badb2febb318cf1",
  base_uri: "http://api.themoviedb.org/3/",
  images_uri: "http://image.tmdb.org/t/p/w500",
  timeout: 5000,
  language: "en-US",
};

function generateQuery(options) {
  var myOptions, query, option;

  myOptions = options || {};
  query = "?api_key=" + CONFIG.api_key + "&language=" + CONFIG.language;

  if (Object.keys(myOptions).length > 0) {
    for (option in myOptions) {
      if (
        myOptions.hasOwnProperty(option) &&
        option !== "id" &&
        option !== "body"
      ) {
        query = query + "&" + option + "=" + myOptions[option];
      }
    }
  }
  return query;
}

export async function getMovies(options) {
  const url = CONFIG.base_uri + "discover/movie" + generateQuery(options);
  const data = await (await fetch(url)).json();
  const results = data.results;
  results.forEach(
    (movie) => (movie.image = CONFIG.images_uri + movie.poster_path)
  );
  return results;
}

export async function getMovie(id, options) {
  const url = CONFIG.base_uri + `movie/${id}` + generateQuery(options);

  const movie = await (await fetch(url)).json();

  movie.image = CONFIG.images_uri + movie.poster_path;

  return movie;
}
