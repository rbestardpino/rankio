...
/**
 * Gets the UID of the user with given `username`
 * @param {string} username
 */
export async function usernameToUID(username) {
  const _doc = await getDoc(doc(db, "usernames", username));
  return _doc.exists() ? _doc.data().uid : null;
}
...
/**
 * Get review for user with uid and movieId
 * @param  {string} uid
 * @param  {number} movieId
 */
export async function getReview(uid, movieId) {
  ...
  const _doc = await getDoc(reviewRef);

  return _doc.exists() ? reviewToJSON(_doc) : null;
}
